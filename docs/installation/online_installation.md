...
请自行下载 KubeOperator 最新版本的在线安装包，并复制到目标机器的 /tmp 目录下
...
cd /tmp
# 解压安装包
tar zxvf KubeOperator-release-v1.0.3.tar.gz
```
...
```sh
cd KubeOperator-release-v1.0.3
vim install.conf
```
...
??? info "安装配置文件说明, 如果无特殊需求可以不进行修改采用默认参数安装"
    ```vim
    # 基础配置
    ## KubeOperator 安装目录的上级目录, KubeOperator 将安装在 ${base_dir}/KubeOperator 目录中
    MS_BASE=/opt
    ## KubeOperator 相关组件所使用 Docker 镜像的镜像地址前缀
    MS_PREFIX=''
    ## KubeOperator 相关组件所使用 Docker 镜像的镜像标签
    MS_TAG=dev
    ## 安装模式, 支持的安装模式有 allinone | server | node-controller 三种
    MS_MODE=allinone
    ## KubeOperator Web 服务的监听端口
    MS_PORT=8081
...
```sh
# 进入项目目录
cd KubeOperator-release-v1.x.y
# 运行安装脚本
/bin/bash install.sh
# 查看 KubeOperator 状态
msctl status
